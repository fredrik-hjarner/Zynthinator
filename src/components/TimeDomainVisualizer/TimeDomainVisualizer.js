...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    uiComponent: stateQueries.getAllUiComponents(state)[id]
  };
};

@connect(mapStateToProps)
export class TimeDomainVisualizer extends React.Component {
  componentDidMount() {
    /**
  ...
  render = () => { // eslint-disable-line
    ...
    settingsProps.bitsToRecordOption = R.map(
      val => ({
        key: `${val}`,
        value: `${val}`,
